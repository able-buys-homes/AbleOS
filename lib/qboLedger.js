...
/**
 * Temporary. Resolves a payment method exactly the way postPayment does, but
 * returns what it found instead of swallowing it - so a silent failure can be
 * read from outside rather than guessed at. Delete once this is understood.
 */
export async function debugPaymentMethod(method) {
    const mapped = QBO_PAYMENT_METHOD[String(method ?? "")] ?? null;

    if (!mapped) return { method, mapped: null, id: null };

    try {
        const id = await ensurePaymentMethod(mapped.name, mapped.card);
        return { method, mapped: mapped.name, id: id ?? null };
    } catch (err) {
        return { method, mapped: mapped.name, error: err?.message ?? String(err) };
    }
}
